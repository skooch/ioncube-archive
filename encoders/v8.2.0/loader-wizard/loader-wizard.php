...
{
    return "The function you have tried to access has been disabled.";
}
    ...
    if (!empty($download)) {
        ...
        if (empty($ini_file_name)) {
			...
			if (!preg_match('/^.*\.ini$/',$ini_file_name) || preg_match('/\//',$ini_file_name) || preg_match('/\\/',$ini_file_name)) {
				...
				$ini_file_name = basename($ini_file_name);
			}
		}
...
{
	$filtered_var = (function_exists('filter_input'))?filter_input(INPUT_SERVER,$server_var,FILTER_SANITIZE_URL):$_SERVER[$server_var];
	$res = htmlspecialchars($filtered_var, ENT_QUOTES, "UTF-8");
	return $res;
}
